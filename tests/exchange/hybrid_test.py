...
def test_open_close_exchange(mock_redis) -> None:
    with UnboundHybridExchangeClient(redis_host='localhost', redis_port=0).bind() as exchange:
...
def test_send_to_mailbox_direct(mock_redis, caplog) -> None:
    ...
    exchange = UnboundHybridExchangeClient(redis_host='localhost', redis_port=0)
    with exchange.bind() as client_1:
        with exchange.bind(handler=EMPTY_HANDLER) as client_2:
            message = PingRequest(src=client_1.mailbox_id, dest=client_2.mailbox_id)
            for _ in range(3):
                client_1.send(client_2.mailbox_id, message)
                assert client_2.recv(timeout=TEST_CONNECTION_TIMEOUT) == message
...
def test_mailbox_recv_closed(mock_redis) -> None:
    with UnboundHybridExchangeClient(redis_host='localhost', redis_port=0).bind() as exchange:
        ...
        with exchange.clone().bind(mailbox_id=aid, handler=EMPTY_HANDLER) as mailbox:
            with pytest.raises(MailboxClosedError):
...
def test_mailbox_redis_error_logging(mock_redis, caplog) -> None:
    ...
    with mock.patch(
        ...
        with UnboundHybridExchangeClient(redis_host='localhost', redis_port=0).bind() as exchange:
            ...
            assert any(
                f'Error in redis watcher thread for {exchange.mailbox_id}' in record.message
...
def test_send_to_mailbox_bad_cached_address(mock_redis) -> None:
    ...
    with BoundHybridExchangeClient(redis_host='localhost', redis_port=0) as client1:
        aid = client1.register_agent(EmptyBehavior)

        with BoundHybridExchangeClient(redis_host='localhost', redis_port=0, port=port1, mailbox_id=aid, handler=EMPTY_HANDLER) as client2:
            message = PingRequest(src=client1.mailbox_id, dest=client2.mailbox_id)
        ...
        with BoundHybridExchangeClient(redis_host='localhost', redis_port=0, port=port2, mailbox_id=aid, handler=EMPTY_HANDLER) as client2:
